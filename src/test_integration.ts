...
async function main() {
    ...
    const content = reviewers.content as any[];
    ...
    const content2 = reviewers2.content as any[];
    const list2 = JSON.parse(content2[0].text);
    console.log("Reviewers after add:", list2.map((r: any) => r.name));

    if (list2.find((r: any) => r.name === "TestAgent")) {
